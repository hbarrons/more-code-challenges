...
function diagonalDifference (arr) {
  let rtl = Math.abs(arr[0][2] + arr[1][1] + arr[2][0])
  return Math.abs(arr[0][0] + arr[1][1] + arr[2][2] - rtl)
}
...
function miniMaxSum (arr){
  ...
  min = (sortedArr.shift()).reduce((a,b)=> a + b)
  max = (sortedArr.pop()).reduce((a,b)=> a + b)
  minMaxArr = [min, max]
...
function timeConversion (timeAmPm){
  amPm = timeAmPm.substring(8)
  timeArr = timeAmPm.split(':')
  if (amPm === 'PM') {
    timeArr[0] = String(parseInt(timeArr[0]) + 12)
    militaryTime = ((timeArr.toString()).substring(0,8)).replaceAll(',', ':')
  } else if (amPm === 'AM' && timeArr[0] === '12') {
    timeArr[0] = String(parseInt(timeArr[0]) - 12 + '0')
    militaryTime = ((timeArr.toString()).substring(0,8)).replaceAll(',', ':')
  } else {
    militaryTime = ((timeArr.toString()).substring(0,8)).replaceAll(',', ':')
  }
